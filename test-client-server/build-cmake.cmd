...
REM 配置 CMake（Debug 版本）
echo 正在配置 CMake（Debug 版本）...
cmake -B build -S . -G "Visual Studio 16 2019" -A x64 -DCMAKE_BUILD_TYPE=Debug
...
echo.
echo 可执行文件位置:
echo   build\bin\Debug\tcp-server.exe
echo   build\bin\Debug\tcp-client.exe
echo.

REM 检查是否生成了可执行文件
if exist "build\bin\Debug\tcp-server.exe" (
    echo tcp-server.exe 已成功生成！
) else (
    echo [93m警告: tcp-server.exe 未找到！[0m
)

if exist "build\bin\Debug\tcp-client.exe" (
